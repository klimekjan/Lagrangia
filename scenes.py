...
class MenuScene():
    ...
    def draw(self, screen: pygame.Surface):  # drawing the menu screen
        ...
        for i, (title, sub, accent, _) in enumerate(self.CARDS):   # calling draw_card function
            self._draw_card(screen, i, title, sub, accent, self._card_rect(i).collidepoint(mouse))

        label = self._f_hint.render("Select a mode above   ·   ESC to quit", True, C.C_LABEL_DIM)  # bottom label
        screen.blit(label, (cx - label.get_width() // 2, C.WINDOW_H - 38))
    ...
    def _draw_card(self, screen, index, title, subtitle, accent, hovered):
        ...
        for surf, y_off in [
            (self._f_card_t.render(title, True, C.C_TITLE),  60),
...
class ExplorerScene():
    ...
    def _draw_panel(self, screen: pygame.Surface):
        ...
        blit_centred(self._f_title.render("[ EXPLORER MODE ]", True, C.C_ACCENT), 42)
        sep(64)
...
class ChallengeScene():
    ...
    def _draw_panel(self, screen: pygame.Surface) -> None:
        ...
        if self._air_on:
            t      = self._air_thread
            status = ("Raise index finger to draw" if (t and t.ready)
